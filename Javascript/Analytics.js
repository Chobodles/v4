...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadDocumentChart() {
    ...
    try {
      var res = await fetch(
        "php/GetAnalytics.php?type=documents&month=" + month + "&year=" + year,
      );
      var data = await res.json();
      hideDocLoading();
      ...
      if (!data.success || !data.data || data.data.length === 0) {
        ...
        if (!empty) {
          ...
          empty.innerHTML =
            '<i class="fa fa-inbox"></i><span>No document requests found for ' +
            MONTHS[month] +
            " " +
            year +
            ".</span>";
          ...
          empty.querySelector("span").textContent =
            "No document requests found for " +
            MONTHS[month] +
            " " +
            year +
            ".";
      ...
      var rawLabels = data.data.map(function (r) {
        return r.document_type || "Unknown";
      });
      var wrappedLabels = rawLabels.map(function (l) {
        return wrapLabel(l, 20);
      });
      var values = data.data.map(function (r) {
      ...
      if (docChartInstance) {
        docChartInstance.destroy();
        docChartInstance = null;
      }
          ...
          datasets: [
            {
              label: "Requests",
              data: values,
              backgroundColor: rawLabels.map(function (_, i) {
                return DOC_COLORS[i % DOC_COLORS.length] + "cc";
              }),
              borderColor: rawLabels.map(function (_, i) {
                return DOC_COLORS[i % DOC_COLORS.length];
              }),
              borderWidth: 2,
              borderRadius: 8,
              borderSkipped: false,
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                title: function (ctx) {
                  return rawLabels[ctx[0].dataIndex];
                },
                label: function (ctx) {
                  return (
                    " " +
      ...
      document.getElementById("doc-top-val").textContent =
        rawLabels[maxIdx] || "—";
  ...
  async function loadBlotterChart() {
    ...
    try {
      var res = await fetch(
        "php/GetAnalytics.php?type=blotter_monthly&year=" + year,
      );
      var data = await res.json();
      hideBlotterLoading();
      ...
      if (!data.success) {
        ...
        if (!empty) {
          ...
          empty.className = "chart-empty";
          empty.innerHTML =
            '<i class="fa fa-inbox"></i><span>No blotter data found for ' +
            year +
            ".</span>";
          area.appendChild(empty);
        } else {
          empty.querySelector("span").textContent =
            "No blotter data found for " + year + ".";
          empty.style.display = "flex";
        }
        hide("blotter-canvas-wrapper");
        hide("blotter-summary");
      ...
      var monthLabels = data.data.map(function (r) {
        return MONTHS[r.month_num].substring(0, 3);
      });
      var totals = data.data.map(function (r) {
        return parseInt(r.total);
      });
      var resolved = data.data.map(function (r) {
        return parseInt(r.resolved);
      });
      var escalated = data.data.map(function (r) {
        return parseInt(r.escalated);
      });
      var dismissed = data.data.map(function (r) {
        return parseInt(r.dismissed);
      });
      var pending = data.data.map(function (r) {
        return parseInt(r.pending);
      });

      var grandTotal = totals.reduce(function (a, b) {
        return a + b;
      }, 0);
      var grandResolved = resolved.reduce(function (a, b) {
        return a + b;
      }, 0);
      var grandEscalated = escalated.reduce(function (a, b) {
        return a + b;
      }, 0);
      var grandDismissed = dismissed.reduce(function (a, b) {
        return a + b;
      }, 0);
      var grandPending = pending.reduce(function (a, b) {
        return a + b;
      }, 0);
      ...
      if (blotterChartInstance) {
        blotterChartInstance.destroy();
        blotterChartInstance = null;
      }
          ...
          datasets: [
            {
              label: "Resolved",
              data: resolved,
              backgroundColor: BLOTTER_COLORS.resolved.bg,
              borderColor: BLOTTER_COLORS.resolved.border,
              borderWidth: 2,
              borderRadius: 4,
              borderSkipped: false,
              barPercentage: 0.85,
            ...
            },
            {
              label: "Escalated",
              data: escalated,
              backgroundColor: BLOTTER_COLORS.escalated.bg,
              borderColor: BLOTTER_COLORS.escalated.border,
              borderWidth: 2,
              borderRadius: 4,
              borderSkipped: false,
              barPercentage: 0.85,
            ...
            },
            {
              label: "Dismissed",
              data: dismissed,
              backgroundColor: BLOTTER_COLORS.dismissed.bg,
              borderColor: BLOTTER_COLORS.dismissed.border,
              borderWidth: 2,
              borderRadius: 4,
              borderSkipped: false,
              barPercentage: 0.85,
            ...
            },
            {
              label: "Pending",
              data: pending,
              backgroundColor: BLOTTER_COLORS.pending.bg,
              borderColor: BLOTTER_COLORS.pending.border,
              borderWidth: 2,
              borderRadius: 4,
              borderSkipped: false,
              barPercentage: 0.85,
      ...
      document.getElementById("blotter-resolved-val").textContent =
        grandResolved;
      document.getElementById("blotter-escalated-val").textContent =
        grandEscalated;
      document.getElementById("blotter-dismissed-val").textContent =
        grandDismissed;
  ...
  document
    .getElementById("doc-load-btn")
    .addEventListener("click", loadDocumentChart);
  document
    .getElementById("blotter-load-btn")
    .addEventListener("click", loadBlotterChart);
